...
export class ProductosControllers{
    ...
    async deactive(@Param('id') id: string): Promise<void>{
        await this.productosServices.deactivate(id);
    }
    ...
    @Put('update/:id')
    async udpate(@Param('id') id: string, @Body() updateProductosDto: UpdateProductoDto): Promise<Productos>{
    ...
    async udpatePartial(@Param('id') id: string, @Body() updateProductosDto: UpdateProductoDto): Promise<Productos>{
